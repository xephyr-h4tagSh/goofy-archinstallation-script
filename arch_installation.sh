...
sleep 1
echo "just a quick warning: i made this for fun, if you don't want your disk to be erased by my mildly shitty scripting, don't use it."
sleep 1
echo "oh and this should only be used in the arch installation environment"
sleep 3
STATION=$(iwctl device list | awk 'NR==4 {print $2}')
iwctl station "$STATION" scan
sleep 1
echo "ok do you want to connect to which network"
mapfile -t NETWORKS < <(iwctl station "$STATION" get-networks | stdbuf -oL tr -d '\r' | awk 'NR > 4 {print $2}' | sed 's/\x1b\[[0-9;]*m//g')
PS3="enter thenumber of which netowrk you want to connect to: "
select SSID in "${NETWORKS[@]}"; do
  if [ -n "$SSID" ]; then
    echo "you chose $SSID"
    read -rs -p "enter your stupid password for $SSID: " PASS
    echo ""
    iwctl --passphrase "$PASS" station "$STATION" connect "$SSID"
    break
  else
    echo "invalid selection. try again next time D:"
    sleep 2
    exit 1
  fi

echo "ok great gng i should have connected you to the internet"
sleep 2
...
if [ -b "$DISK" ]; then
    ...
    if [ "$CONFIRMATION" = "Y" ]; then
        ...
        echo "disk $DISK is now clean. ready for partitioning motherfuckers"
    else
        echo "aw ok"
        sleep 3
        exit 1
